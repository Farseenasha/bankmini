...
def user_register(request):
    if request.method == "POST":
        ...
        User = Customs.objects.create_user(first_name=Name,address=Address,adhar_no=Adhar_no,image=Image,
                                           account_no=Account_no,dob=Dob,pancard_no=Pancard_no,amount=Amount,
...
def withraw(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if data.amount <= 1000:
                return render(request,'user/withdraw.html',{'withdraw':"can't withdraw money.minimum balance required 1000rs"})
            if withdraw < 200:
                return render(request,'user/withdraw.html',{'withdrawarning':"can't withdraw money.minimum amount required 200rs"})
...
def deposite(request):            
    ...
    if request.method == "POST":
        ...
        try:
            data.account_no == Account_no
            if deposite < 200:
                return render(request,'user/deposite.html',{'depositewarning':"can't withdraw money.minimum amount required 200rs"})
...
def user_no(request):
    user_no = Customs.objects.filter(user_type="user").count()
    return render(request,'bank/user_no.html',{'user_no':user_no})
